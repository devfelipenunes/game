...
contract RobotsNFT is ERC721, OwnableRoles {
    ...
    function burn(uint256 robotId) external {
        ...
        unchecked {
            --totalSupply;
        } // saves gas, no underflow, because cannot burn nonexisting robot
    ...
    function getStats(
        uint256 robotId
    ) external view returns (uint8 attack, uint8 defence, uint32 readyTime) {
        // require(_exists(robotId), "The robot doesn't exist!"); // remove, if 'delete robots[]' in burn(uint256) is removed
        return (
            robots[robotId].attack,
            robots[robotId].defence,
            robots[robotId].readyTime
        );
    }
}
