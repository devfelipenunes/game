...
contract Growing is Factory {
     ...
     */
    function combineRobots(
        uint256 robotId1,
        uint256 robotId2
    ) external virtual returns (uint256 robotId) {
    ...
    function multiplyRobots(
        ...
        (uint8 attack1, uint8 defence1, uint32 readyTime1) = nft.getStats(
            robotId1
        );
        (uint8 attack2, uint8 defence2, uint32 readyTime2) = nft.getStats(
            robotId2
        );
