...
contract Utils is UUPSUpgradeable, OwnableUpgradeable, IERC721Receiver {
    ...
    event newCombiningFeeEvent(
        uint128 oldCombiningFee,
        uint128 newCombiningFee
    );
    event newMultiplyingTaxEvent(
        uint8 oldMultiplyingTax,
        uint8 newMultiplyingTax
    );
    event newMultiplyingFeeEvent(
        uint128 oldMultiplyingFee,
        uint128 newMultiplyingFee
    );
    event newMultiplyingCooldownEvent(
        uint32 oldMultiplyingCooldown,
        uint32 newMultiplyingCooldown
    );
    event robotMintEvent(uint256 robotId, uint8 attack, uint8 defence);
    event withdrawEvent(uint256 amountEth, uint256 amountToken);
    event newMintingFeeInEthEvent(
        uint128 oldMintingFeeInEth,
        uint128 newMintingFeeInEth
    );
    event newMintingFeeInTokenEvent(
        uint128 oldMintingFeeInToken,
        uint128 newMintingFeeInToken
    );
    event createArenaEvent(
    ...
    event fightingEvent(
        ...
        uint256 winnerRobotId,
        address indexed loser,
        uint256 loserRobotId
    );
    ...
    function setMintingFeeInEth(
        uint128 _newMintingFeeInEth
    ) external onlyOwner {
        emit newMintingFeeInEthEvent(mintingFeeInEth, _newMintingFeeInEth);
        mintingFeeInEth = _newMintingFeeInEth;
    }

    function setMintingFeeInToken(
        uint128 _newMintingFeeInToken
    ) external onlyOwner {
        emit newMintingFeeInTokenEvent(
            mintingFeeInToken,
            _newMintingFeeInToken
        );
